...
public class Desugar implements Expr.Visitor<Expr>, Stmt.Visitor<Stmt> {
    ...
    bodyPlusIncr.add(stmt.body);
    if (stmt.incr != null) {
        bodyPlusIncr.add(new Stmt.Expression(stmt.incr));
    }

    Stmt whileStmt = new Stmt.While(stmt.cond, new Stmt.Block(bodyPlusIncr));

    if (stmt.init != null) {
        List<Stmt> blockStatements = new ArrayList<>();
        blockStatements.add(new Stmt.Expression(stmt.init));
